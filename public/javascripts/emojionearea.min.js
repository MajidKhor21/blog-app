...
!(function (a, b, c) {
  ...
  function o(a, b, d, e) {
    (e =
      ...
      c.each(d, function (g, h) {
        (g = c.isArray(d) ? h : g),
          (f[a.id][h] || (f[a.id][h] = [])).push([b, g, e]);
      });
  }
  function p(a, b, c) {
    return a
      .replace("{name}", c || "")
      .replace("{img}", g.imagePathPNG + (2 > m ? b.toUpperCase() : b) + ".png")
      .replace("{uni}", b)
  ...
  function q(a, b, c) {
    return a.replace(/:?\+?[\w_\-]+:?/g, function (a) {
      ...
      return d
        ? (m > 3 && (d = d.unicode), p(b, d[d.length - 1], a))
        : c
        ? ""
        : a;
    });
  ...
  function u(a) {
    if (a.filters) {
      ...
      c.each(a.filters, function (a, d) {
        return !t(d) || c.isEmptyObject(d)
          ? void delete b[a]
          : void c.each(d, function (c, d) {
              b[a][c] = d;
            });
      }),
        (a.filters = b);
  ...
  b.getSelection && a.createRange
    ? ((v = function (a) {
        var c = b.getSelection().getRangeAt(0),
          d = c.cloneRange();
        return (
        ...
        while (!j && (h = g.pop()))
          if (3 == h.nodeType) {
            ...
            !i &&
              d >= e &&
              k >= d &&
              (f.setStart(h, d - e), f.setEnd(h, d - e), (j = !0)),
              (e = k);
          } else {
  ...
  var x,
    y = b.emojioneVersion || "2.1.4",
    z = "https://cdnjs.cloudflare.com/ajax/libs/emojione/";
  function A() {
    var a = g.cacheBustParam;
    return t(g.jsEscapeMap)
      ? "?v=1.2.4" === a
        ? "2.0.0"
        : "?v=2.0.1" === a
        ? "2.1.0"
  ...
  !g || B(A()) < 2
    ? c.getScript(z + y + "/lib/js/emojione.min.js", function () {
        (g = b.emojione), (y = A()), (m = B(y)), (z += y + "/assets");
        var d = z + "/sprites/emojione.sprites.css";
        a.createStyleSheet
          ? a.createStyleSheet(d)
          : c("<link/>", { rel: "stylesheet", href: d }).appendTo("head");
        while (h.length) h.shift().call();
      })
    : ((y = A()), (m = B(y)), (z += y + "/assets")),
    i(function () {
      (g.imagePathPNG = z + "/png/"),
        (x = new RegExp(
  ...
  function D(a, b) {
    return (
      ...
      b.shortnames && (a = g.shortnameToUnicode(a)),
      C(a, b.emojiTemplate)
        .replace(/\t/g, "&nbsp;&nbsp;&nbsp;&nbsp;")
        .replace(/  /g, "&nbsp;&nbsp;")
    );
  ...
  function F() {
    ...
    c !== b &&
      (a.button.css({ marginRight: b }),
      "top" === a.pickerPosition &&
        a.picker.css({ right: parseInt(a.picker.css("right")) - c + b }));
  }
  function G() {
    var a = this;
    if (!a.sprite && a.lasyEmoji[0]) {
      ...
      a.lasyEmoji.each(function () {
        ...
        e > b &&
          d > e &&
          a.attr("src", a.data("src")).removeClass("lazy-emoji");
      }),
  ...
  function J(d, e, f) {
    (f = u(f)),
      (d.sprite = f.sprite && 3 > m),
      (d.inline = null === f.inline ? e.is("INPUT") : f.inline),
      (d.shortnames = f.shortnames),
      (d.pickerPosition = f.pickerPosition),
      (d.saveEmojisAs = f.saveEmojisAs),
      (d.emojiTemplate =
        '<img alt="{alt}" class="emojione' +
        (d.sprite ? '-{uni}" src="' + j + '"/>' : 'emoji" src="{img}"/>'));
    var h = e.is("TEXTAREA") || e.is("INPUT") ? "val" : "text",
      i,
      k,
      p,
      s,
      ...
      B,
      C = I(
        { class: l + " " + (e.attr("class") || ""), role: "application" },
        (i = d.editor = I("editor").attr({
          contenteditable: !0,
          placeholder:
            f.placeholder ||
            e.data("placeholder") ||
        ...
        (k = d.button = I("button", I("button-open"), I("button-close")).attr(
          "title",
          f.buttonTitle
        )),
        (p = d.picker = I(
          ...
          .addClass(H("picker-position-" + f.pickerPosition, !0))
          .addClass(H("filters-position-" + f.filtersPosition, !0))
          .addClass("hidden"))
      );
    c.each(f.attributes, function (a, b) {
      i.attr(a, b);
    }),
      c.each(f.filters, function (a, b) {
        ...
        do {
          var g = I("category").attr({ name: a, "data-tone": e }).appendTo(z),
            h = b.emoji.replace(/[\s,;]+/g, "|");
          e > 0 &&
            (g.hide(),
            (h = h.split("|").join("_tone" + e + "|") + "_tone" + e)),
            (h = q(
              h,
              d.sprite
      ...
      (f.filters = null),
      d.sprite || (d.lasyEmoji = z.find(".lazy-emoji")),
      (y = x.find(H("filter"))),
      y.eq(0).addClass("active"),
      (A = z.find(H("category"))),
      f.container ? c(f.container).wrapInner(C) : C.insertAfter(e),
      f.hideSource && e.hide(),
      d.setText(e[h]()),
      e[h](d.getText()),
      F.apply(d),
      o(d, z.find(".emojibtn"), { click: "emojibtn.click" }),
      o(d, b, { resize: "!resize" }),
      o(d, s.children(), { click: "tone.click" }),
      o(d, [p, k], { mousedown: "!mousedown" }, i),
      o(d, k, { click: "button.click" }),
      o(d, i, { paste: "!paste" }, i),
      o(d, i, ["focus", "blur"], function () {
        return d.stayFocused ? !1 : i;
      }),
      o(d, p, {
      ...
      o(d, p.find(".emojionearea-filter"), { click: "filter.click" }),
      t(f.events) &&
    ...
    if (
      (B.on("scroll", function () {
        if (!J && (G.call(d), B.is(":not(.skinnable)"))) {
          var a = A.eq(0),
            b = B.offset().top;
          A.each(function (d, e) {
            return c(e).offset().top - b >= 10 ? !1 : void (a = c(e));
          });
          var e = y.filter('[data-filter="' + a.attr("name") + '"]');
          e[0] &&
            !e.is(".active") &&
            (y.removeClass("active"), e.addClass("active"));
        }
      }),
      d
        .on("@filter.click", function (a) {
          ...
          if (B.is(".skinnable")) {
            if (b) return;
            s.children().eq(0).click();
          }
          (J = !0),
            b ||
              (y.filter(".active").removeClass("active"), a.addClass("active"));
          var c = A.filter('[name="' + a.data("filter") + '"]').offset().top,
            e = B.scrollTop(),
            f = B.offset().top;
          B.stop().animate(
            { scrollTop: c + e - f - 2 },
        ...
        .on("@tone.click", function (a) {
          ...
          b
            ? (B.addClass("skinnable"),
              A.hide()
                .filter("[data-tone=" + b + "]")
                .show())
            : (B.removeClass("skinnable"),
              A.hide().filter("[data-tone=0]").show()),
            G.call(d);
        })
        ...
        .on("@!paste", function (a) {
          ...
          b.setTimeout(function () {
            ...
            var j = c("#" + b),
              k = j.offset().top - a.offset().top,
              l = a.height();
            (f + k >= l || f > k) && a.scrollTop(f + k - (2 * l) / 3),
              j.remove(),
              (d.stayFocused = !1),
              F.apply(d),
              n(d, "paste", [a, h, i]);
        ...
        .on("@emojibtn.click", function (a) {
          C.is(".focused") || i.focus(),
            v(i[0]),
            r(q(a.data("name"), d.emojiTemplate));
        })
        .on("@!resize @keyup @emojibtn.click", F)
        .on("@!mousedown", function (a, b) {
          return C.is(".focused") || a.focus(), b.preventDefault(), !1;
        })
        .on("@change", function (a) {
          var b = a.html().replace(/<\/?(?:div|span|p)[^>]*>/gi, "");
          (b.length && !/^<br[^>]*>$/i.test(b)) ||
            d.editor.html((d.content = "")),
            e[h](d.getText());
        })
        .on("@focus", function () {
          C.addClass("focused");
        ...
        .on("@blur", function (a) {
          C.removeClass("focused"), f.hidePickerOnBlur && d.hidePicker();
          var b = a.html();
          d.content !== b
            ? ((d.content = b), n(d, "change", [a]), e.blur().trigger("change"))
            : e.blur();
        }),
      f.shortcuts &&
        d.on("@keydown", function (a, b) {
          b.ctrlKey ||
            (9 == b.which
              ? (b.preventDefault(), k.click())
              : 27 == b.which &&
                (b.preventDefault(), k.is(".active") && d.hidePicker()));
        }),
      f.autocomplete)
    ) {
      var K = function () {
      ...
      return this.each(function () {
        return this.emojioneArea
          ...
          : (c.data(
              this,
              "emojioneArea",
              (this.emojioneArea = new K(c(this), a))
            ),
            this.emojioneArea);
      });
    });
