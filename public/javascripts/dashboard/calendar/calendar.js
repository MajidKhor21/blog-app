...
let _day = 1;
let _currDay;
let _jlcal = new JalaliCal();
let _hjcal = new HijriCal();
...
function InitCalendar(table) {
  ...
  let jalaliDate = _jlcal.toJalaali(year, month, day);
  _year = jalaliDate.jy;
...
function UpdateCal(table, year, month, day) {
  ...
  for (let index = dayOfWeek; index < daysInMonth + dayOfWeek; index++) {
...
$(".fc-table td").on("click", function (e) {
  ...
  if ($(this).attr("data-jl") && $(this).attr("data-gr")) {
    let title = "",
      content = "";
    _currDay = $(this);
    console.log(_currDay);
    _currDay.css("border", "2px solid red");
    let jl = $(this).attr("data-jl").split("-");
    let wc = $(this).attr("data-gr").split("-");
    let hj = $(this).attr("data-hj").split("-");
    $.ajax({
      url:
